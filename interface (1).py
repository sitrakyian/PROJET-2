...
def _maj_indicateurs():
    ...
    for i, cat in enumerate(N.CATEGORIES):
        ...
        if etat == "OK":
            ...
            etat    = "MANQUE"
            couleur = ROUGE_KO
        lbl_cat_point[i].config(fg=couleur)
        lbl_cat_etat[i].config(text=etat, fg=couleur)
        lbl_cat_nb[i].config(text=str(nb) + "  " + str(pct) + "%")
...
def _ligne_champ(parent, row_num, libelle, sv):
    Label(parent, text=libelle, font=("Helvetica", 11),
...
def _ligne_menu(parent, row_num, libelle, sv):
    ...
    menu.config(font=("Helvetica", 11), bg=BLANC,
...
def _boutons_form(parent, row_num, texte_ok, cmd_ok, cmd_annuler, couleur_ok=None):
    ...
    Button(f, text=texte_ok, bg=couleur_ok, fg=BLANC,
           font=("Helvetica", 12), width=14, relief="flat", cursor="hand2",
...
def ouvrir_confirmation_suppression(index):
    ...
    Label(win,
          ...
          font=("Helvetica", 11), bg=BLANC, fg=GRIS_TEXTE, justify="left"
    ...
    Button(frame_btn, text="Oui, supprimer",
           bg=ROUGE_KO, fg=BLANC, font=("Helvetica", 12), width=14,
           relief="flat", cursor="hand2",
           command=lambda: _confirmer()
           ).grid(row=0, column=0, padx=12)

    Button(frame_btn, text="Non",
           bg=GRIS_FOND, fg=NOIR_TITRE, font=("Helvetica", 12), width=10,
           relief="flat", cursor="hand2",
...
ma_fenetre.config(bg=GRIS_FOND)
...
Label(frame_entete,
      ...
      bg=BLANC, fg=NOIR_TITRE
      ).grid(row=0, column=0, pady=(2,2),padx=14, sticky="w")
...
Button(frame_entete,
       ...
       command=lambda: ouvrir_formulaire_ajout()
       ).grid(row=0, column=2, padx=14, rowspan=2)
...
# ROW 3 : Bloc services
# ----------------------------------------------------------
frame_services = Frame(ma_fenetre, bg=GRIS_FOND)
...
# ----------------------------------------------------------
frame_cats = ctk.CTkFrame(ma_fenetre,fg_color=BLANC)
...
# ----------------------------------------------------------
frame_tableau = Frame(ma_fenetre, bg=GRIS_FOND)
frame_tableau.grid(row=5, column=0, sticky="nsew", padx=12, pady=(4, 0))
...
# ----------------------------------------------------------
frame_actions = Frame(ma_fenetre, bg=GRIS_FOND)
frame_actions.grid(row=6, column=0, sticky="w", padx=12, pady=(4, 12))

Button(frame_actions, text="Modifier",
       font=("Helvetica", 10), bg=BLANC, fg=NOIR_TITRE,
       relief="solid", bd=1, padx=12, pady=5, cursor="hand2",
       command=lambda: action_modifier()
       ).grid(row=0, column=0, padx=(0, 8))

Button(frame_actions, text="Supprimer",
       font=("Helvetica", 10), bg=BLANC, fg=ROUGE_KO,
       relief="solid", bd=1, padx=12, pady=5, cursor="hand2",
